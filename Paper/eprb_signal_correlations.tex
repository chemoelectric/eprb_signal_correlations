...
{\bf{THE FOLLOWING REALLY COULD USE REVISION. IT DOES NOT SEEM
    RIGHT. I SHOULD REWRITE USING SOME COORDINATE-FREE
    REPRESENTATION. MIND YOU, THE SOLUTION IS CORRECT OR THE
    SIMULATIONS WOULD NOT WORK. I MEAN THAT I AM TRYING TO WRITE
    COORDINATE-FREE ARGUMENTS IN COORDINATE-UNFREE NOTATIONS, AND NOT
    GETTING IT QUITE RIGHT.}}  Note also that all angles will be in
some relation relative to each other, and positioned as a whole with
respect to some ``landmark.'' We will call the landmark $\phi_1$ and
associate it, purely for convenience, with the ``calibration'' of
channel~1. The landmark may be any angle, and thus
``$\phi_1\in[0,2\pi]$'' will often appear as a condition.
