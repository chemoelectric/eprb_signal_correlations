...
experiment be properly designed. This inequality applies a test that
itself is not invariant under rotations of the apparatus. For
Bell-test angles, in the case of photon experiments, a rotation by
$\pi/8$ should see a drop in magnitude of the CHSH ``correlation
function'' from $\sqrt{1\!/2}$ to~zero, so~$|S|$ (in the ideal) adds
up to~$\sqrt2$ instead of~$2\sqrt2$. Yet experimenters report positive
results. Therefore not only is the inequality itself a failure of
scientific methods, so must be the experiments.
